...
stacks.get(AWS, name, function(err, stack) {
  ...
  if (!program.version) {
    ...
    traffic.list(AWS, stack.Resources['ResultsELB'].PhysicalResourceId, function(err, instances) {
      if (err) {
        console.error(err);
        process.exit(1);
      }
      ...
      instances.forEach(function(instance) {
        console.log(instance.InstanceId, instance.ELBState, instance.State.Name, instance.Tags['zetta:results:version'], instance.Tags['aws:autoscaling:groupName'])
      });
    });
    return;
  }
  ...
  results.list(AWS, name, function(err, versions) {
    ...
    var opts = { 
      ...
      elbName: stack.Resources['ResultsELB'].PhysicalResourceId,
      stack: name
    };
